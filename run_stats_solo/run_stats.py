...
def create_new_run_stat(api, itemnr, name):
    ...
    state = 'Folyamatban'
    sql_insert_stat_query = insert_stat_query.format(**{'name': used_name, 'api': api, 'item_nr': itemnr, 'state': state})
    # insert(sql_insert_stat_query)
    #
    # query = '''select id from run_stat where stat_name = '{name}' '''.format(**{'name': 'auto'})
...
values ('{name}', '{api}', '{item_nr}', '{state}', 0, {median_iqr}, {multiplier_iqr}, {multiplier_std})'''
    ...
    # sql_get_id = '''select id from ts_stat where stat_name = '{name}' '''.format(**{'name': 'auto'})
