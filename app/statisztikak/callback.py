...
def stat_itemnr_options(api: str) -> utils.Options:
    ...
    recipe_view = 'EFR_S0000034.k1_apimpp1_gb_batch_view_tunde_view' if api == '1' else 'EFR_S0000067.k1_apimpp2_gb_batch_view_tunde_view'
...
def stat_chosen_product(itemnr: Optional[str]) -> str:
    ...
    query = f"select distinct ProductName from EFR_APIbatch.k1_apimpp_gb_sap_tunde_view where ItemNr = '{itemnr.strip()}'"
...
def stat_time_chkbxs(itemnr: str, api: str) -> utils.Options:
    if api == '1':
        recipe_view = 'EFR_S0000034.k1_apimpp1_gb_batch_view_tunde_view'
    elif api == '2':
        recipe_view = 'EFR_S0000067.k1_apimpp2_gb_batch_view_tunde_view'
    else:
        return []
    ...
    options = [{'label': month_row['month'], 'value': month_row['month']}
...
def stat_fill_selected_values(data, selected_row, table):
    ...
    query_params = {'item_nr': item_nr.strip()}
    query = f"select ProductName from EFR_APIbatch.k1_apimpp_gb_sap_tunde_view where ItemNr = '{item_nr}'".format(**query_params)
    product = utils.run_query(query)
    ...
    query = "select distinct concat(charge_nr, ' (', FORMAT(charge_start_date, 'yyyy-MM-dd'), ')') as charge from {table} where stat_id = '{stat_id}'".format(
        **query_params)
...
def insert_df(cursor, df: pd.DataFrame) -> None:
    ...
    df.rename(columns={'OpType': 'Op'}, inplace=True)
    for i, row in df.iterrows():
        for j, column in row.iteritems():
            if isinstance(column, float):
                if abs(column) > 999999999:
                    df.at[i, j] = -100000000.0
...
def unitrecipe_run_stats(api: str, stat_id: int) -> pd.DataFrame:
    ...
    if api == '1':
        recipe_view = 'EFR_S0000034.k1_apimpp1_gb_batch_view_tunde_view'
        unitrecipe_view = 'EFR_S0000034.k1_apimpp_1_gb_unitrecipe_tunde_view'
    elif api == '2':
        recipe_view = 'EFR_S0000067.k1_apimpp2_gb_batch_view_tunde_view'
        unitrecipe_view = 'EFR_S0000067.k1_apimpp_2_gb_unitrecipe_tunde_view'
    else:
        raise ValueError
...
def operation_run_stats(api: str, stat_id: int) -> pd.DataFrame:
    ...
    """
    if api == '1':
        operation_view = 'EFR_S0000034.k1_apimpp1_gb_operation_view_tunde_view'
        unitrecipe_view = 'EFR_S0000034.k1_apimpp_1_gb_unitrecipe_tunde_view'
    elif api == '2':
        operation_view = 'EFR_S0000067.k1_apimpp2_gb_operation_view_tunde_view'
        unitrecipe_view = 'EFR_S0000067.k1_apimpp_2_gb_unitrecipe_tunde_view'
    else:
        raise ValueError
...
--        DATEDIFF(minute, c.started, vu.Started) as total_offset_old,
  ...
  join unitrecipe_run_stats us
    ...
    stats = operations_df.groupby(['RecipeGrpNum', 'RecipeNum', 'UnitRecipeNum', 'OpInst', 'OpType']).agg('describe')
    stats['stat_id'] = stat_id
    stats.columns = [f'{level0}_{level1}' if level1 is not '' else level0
                     for level0, level1 in stats.columns.values]
    stats.drop(labels=['duration_count', 'duration_mean', 'duration_std',
...
def operation_stats(stat_id: int):
    ...
    all_sensor_cols = utils.run_query(sql.query_all_sensor_cols_api)
    all_sensor_cols = [row['COLUMN_NAME'] for row in all_sensor_cols]
    sql_query_operations = sql.query_operations.format(**{'stat_id': stat_id})
    ...
    for idx, op in enumerate(operations):
        ...
        sql_query_op_units_api12 = sql.query_op_units_api.format(**op_identifier_params)
        ...
        for unit in units:
            ...
            query_offset_ts = sql.query_op_offset_ts_api
            query_min_max_dt = sql.query_op_min_max_dt_api
